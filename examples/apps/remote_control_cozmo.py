...
class RobotStateDisplay(pycozmo.annotate.Annotator):
    def apply(self, image, scale):
        ...
        def print_line(text_line):
            text = pycozmo.annotate.ImageText(text_line, position=pycozmo.annotate.TOP_LEFT, outline_color='black', color='lightblue')
...
class RemoteControlCozmo:
    ...
    def try_play_anim(self, anim_name):
        try:
            ...
            return True
        except pycozmo.exceptions.RobotBusy:
            return False
    ...
    def update(self):
        ...
        if len(self.action_queue) > 0:
            ...
            if queued_action(action_args):
                self.action_queue.pop(0)
        # Update gyro
        if self.is_device_gyro_mode_enabled and self.cozmo.gyro:
    ...
    def update_gyro_driving(self):
        pitch, yaw, roll = self.cozmo.gyro.x_y_z
    ...
    try:
        while True:
            if remote_control_cozmo:
                ...
                asyncio.sleep(.1)
    except pycozmo.exceptions.SDKShutdown:
